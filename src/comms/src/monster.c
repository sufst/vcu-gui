...
#include "schema_builder.h" // Generated by `flatcc`.
...
int create_monster_bottom_up(flatcc_builder_t* B, int flexible)
{
    ...
    if (!flexible)
    {
        // Finally, create the monster using the `Monster_create` helper
...
int create_monster_top_down(flatcc_builder_t* B)
{
    ...
    if (1)
    {
        ...
        ns(Monster_weapons_push_create(
            B,
            flatbuffers_string_create_str(B, "Sword"),
            3));
        // We reuse the axe object later. Note that we dereference a pointer
        // because push always returns a short-term pointer to the stored
        // element. We could also have created the axe object first and simply
        // pushed it.
        axe = *ns(
            Monster_weapons_push_create(B,
                                        flatbuffers_string_create_str(B, "Axe"),
                                        5));
    ...
    // Unions can get their type by using a type-specific add/create/start
    // method.
...
int access_monster_buffer(const void* buffer)
{
    ...
    // This is just a const char *, but it also supports a fast length
    // operation.
    ...
    // not always map cleanly to 32-bit represention even with only a few
    // digits: `1.0 == 1.0f`, but `3.2 != 3.2f`.
    test_assert(x == 1.0f);
    test_assert(y == 2.0f);
    test_assert(z == 3.0f);
