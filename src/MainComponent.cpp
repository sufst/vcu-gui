...
MainComponent::MainComponent()
{
    ...
    for (int i = 0; i < interpolationMethods.size(); i++)
    {
        ...
        juce::String methodName
            = ThrottleCurve::getInterpolationMethodName(method);
    ...
    interpolationMethodComboBox.onChange = [this]()
    {
        ...
        auto& method = ThrottleCurve::getAllInterpolationMethods().getReference(
            selectedItem);
        throttleCurveComponent.setInterpolationMethod(method);
    };

    // set callback lambda to be called when profile loaded
    throttleCurveComponent.onProfileLoad
        = [this](ThrottleCurve::InterpolationMethod method)
    {
    ...
    importProfileButton.onClick
        = [this]() { throttleCurveComponent.importProfile(); };

    exportProfileButton.onClick
        = [this]() { throttleCurveComponent.exportProfile(); };

    exportCodeButton.onClick
        = [this]() { throttleCurveComponent.exportCode(); };
